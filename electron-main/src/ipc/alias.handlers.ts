...
import {Alias, AliasData, IncomingAliasData} from "../types";
...
export function registerAliasHandlers(): void {
    ...
    ipcMain.handle('get-aliases', async (event: IpcMainInvokeEvent): Promise<Alias[]> => {
        ...
        try {
            const aliasData: AliasData = await readAliasData();
            return Object.values(aliasData.aliases);
        } catch (error) {
    ...
    ipcMain.on('add-alias', async (event: IpcMainEvent, receivedAliasData: IncomingAliasData) => {
        ...
        try {
            ...
            const nameExists = Object.values(aliasData.aliases).some(alias => alias.name === receivedAliasData.name);
            if (nameExists) {
            ...
            aliasData.aliases[newAlias.id] = newAlias;

            await saveAliasData(aliasData);
            await regenerateAliasShellFile(aliasData);

            console.log(`Alias ${newAlias.name} processed successfully.`);
            event.reply('add-alias-reply', { success: true, alias: newAlias });

            if (store.has('user')) {
                cloudSyncService.uploadAliasToRealtimeDatabase(newAlias);
            }
        } catch (error: any) {
        ...
        if (store.has('user')) {
            try {
                ...
                for (const alias of incomingAliases) {
                    aliasData.aliases[alias.id] = alias;
                }
    ...
    ipcMain.on('update-alias', async (event: IpcMainEvent, idToUpdate: string, updatedAliasData: Alias) => {
        ...
        try {
            ...
            if (!aliasData.aliases[idToUpdate]) {
                throw new Error(`Alias with ID "${idToUpdate}" not found.`);
            }

            const originalName = aliasData.aliases[idToUpdate].name;
            if (originalName !== updatedAliasData.name) {
                const nameConflict = Object.values(aliasData.aliases).some(
                    (a) => a.name === updatedAliasData.name && a.id !== idToUpdate
                );
                if (nameConflict) {
            ...
            updatedAliasData.lastUpdated = new Date().toISOString();
            aliasData.aliases[idToUpdate] = updatedAliasData;
            ...
            console.log(`Alias (ID: ${idToUpdate}) updated successfully.`);
    ...
    ipcMain.on('delete-alias', async (event: IpcMainEvent, idToDelete: string) => {
        ...
        try {
            const aliasData: AliasData = await readAliasData();
            const aliasToDelete = aliasData.aliases[idToDelete];
            const deletedAliasName = aliasToDelete ? aliasToDelete.name : null;

            if (!aliasToDelete) {
                throw new Error(`Alias with ID "${idToDelete}" not found for deletion.`);
            }

            delete aliasData.aliases[idToDelete];
            aliasData.deleted[idToDelete] = {
                id: idToDelete,
