...
const api: ElectronAPI = {
    ...
    getAliases: () => ipcRenderer.invoke('get-aliases') as Promise<Alias[]>, // Return AliasData

    addAlias: (alias: IncomingAliasData) => ipcRenderer.send('add-alias', alias),
    onAddAliasReply: (callback) => ipcRenderer.on('add-alias-reply', (_event, result) => callback(result)),

    updateAlias: (id, alias) => ipcRenderer.send('update-alias', id, alias),
    ...
    syncAliasesFromCloud: (aliasData) => {
        if (!aliasData || !aliasData.aliases) {
            console.error('Invalid AliasData structure:', aliasData);
            return Promise.resolve({ success: false, error: 'Invalid AliasData structure' });
        }
        return ipcRenderer.invoke('sync-aliases-from-cloud', aliasData);
    },
    ...
    onAliasesUpdated: (callback) =>
        ipcRenderer.on('aliases-updated', (_event, aliasData: AliasData) => callback(aliasData)),
